...
#define TIM_Psc_BEEP 													84    //CK_CNT等于fCK_PSC/(PSC[15:0]+1)，可以实现1至65536分频。
...
void Beep_Init()
{
  ...
  GPIO_Init(GPIO_BEEP, &GPIO_InitStructure); 
	...
	TIM_TimeBaseStructure.TIM_Prescaler=TIM_Psc_BEEP-1;  //定时器分频,预分频器
	...
	TIM_OCInitStructure.TIM_Pulse = 0;//比较输出脉冲宽度
